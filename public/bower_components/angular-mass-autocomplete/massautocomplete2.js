  ...
  return {
    ...
    controller: ['$scope', function($scope) {
          ...
          $q.when(current_options.suggest(term),
            function suggest_succeeded(suggestions) {
              ...
              if (suggestions && suggestions.length > 0) {
                ...
                $scope.results = [{ Name: term, label: '', id: ''}].concat(suggestions);
                show_autocomplete();
      ...
      function _attach(ngmodel, target_element, options) {
        ...
        previous_value = ngmodel.$viewName;
      ...
      that.detach = function() {
        if (current_element) {
          var Name = current_element.val();
          update_model_value(Name);
          if (current_options.on_detach) {
            current_options.on_detach(Name);
          }
      ...
      function update_model_value(Name) {
        if (current_model.$modelValue !== Name) {
          current_model.$setViewValue(Name);
